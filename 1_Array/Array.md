...
7. **是否有序**：数组是完全排好序的还是只有一部分排好序？ 如果是，则应该可以进行某种形式的二分查找。 这通常意味着面试官正在寻找比O(N)更快的解决方案。
8. **是否可排序**：有时先对数组进行排序可以大大简化问题。在尝试排序之前，请确保不需要保留数组元素的原来的顺序。
9. **子数组的和/积问题**：若涉及子数组的求和或乘法，考虑**散列，前缀和，前缀积，后缀和，后缀积**等预计算。
10. **O(1)复杂度**：如果给出一个序列并且要求O(1)空间，则可以将该**数组本身用作哈希表**。例如，如果数组的值都在1到N之中，其中N是数组的长度，则用值作为键可以建立一个哈希表，从而可以O(1)确定一个值存在与否。参考剑指Offer第三题：数组中重复的数字。
